...
echo( ________________________________________________________________________
echo.
choice /C:1234 /N /M "[1] Continue [2] Exit [3] Revert Changes [4] Restart to Startup Settings (F7):
if !errorlevel!==2 exit /b
if !errorlevel!==3 goto :dk_revert
if !errorlevel!==4 goto :dk_restart_startup

:dk_restart_startup
cls
echo.
echo %cBlueHL%Restarting to Startup Settings...%cReset%
echo.
echo %cBlueHL%Press F7 to disable driver signature enforcement.%cReset%
echo.
...
set "testsigning="
for /f "tokens=2" %%A in ('bcdedit /enum {current} 2^>nul ^| findstr /i "testsigning"') do (
    if /i "%%A"=="Yes" set "testsigning=1"
)
if defined testsigning (
    echo.
...
if defined dgquery (
    ...
    if defined cgscenario (
        echo.
        echo Checking Credential Guard Scenarios     %cYellow%[Found]%cReset%
        reg add "HKLM\SOFTWARE\ManageVBS" /v CredentialGuardScenario /t REG_DWORD /d 1 /f >nul 2>&1
        reg add "HKLM\SYSTEM\CurrentControlSet\Control\DeviceGuard\Scenarios\CredentialGuard" /v Enabled /t REG_DWORD /d 0 /f >nul 2>&1
        if "!errorlevel!"=="0" (
            echo Disabling Credential Guard Scenarios    %cGreen%[Successful]%cReset%
			set "anythingdisabled=1"
        ) else (
            echo Disabling Credential Guard Scenarios    %cRed%[Failed]%cReset%
...
if not defined anythingdisabled (
    if defined testsigning (
        echo.
        echo %cYellow%All required security features are already disabled, and Test Signing is already enabled.%cReset%
        echo.
        echo %cYellow%Press any key to exit...%cReset%
        pause >nul
        exit /b
...
if "!revert_cgscenario!"=="0x1" (
    ...
    if "!errorlevel!"=="0" (
        echo.
        echo Enabling Credential Guard Scenarios     %cGreen%[Successful]%cReset%
        reg delete "HKLM\SOFTWARE\ManageVBS" /v CredentialGuardScenario /f >nul 2>&1
    ) else (
        echo.
        echo Enabling Credential Guard Scenarios     %cRed%[Failed]%cReset%
        set "haderror=1"
    )
)
